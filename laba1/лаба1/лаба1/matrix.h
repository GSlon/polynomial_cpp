...
class matrix
{
	...
	static_assert(std::is_same<T, int>::value || std::is_same<T,char>::value || std::is_same<T, unsigned char>::value || std::is_same<T, fraction<int>>::value
		|| std::is_same<T, fraction<char>>::value || std::is_same<T, fraction<unsigned char>>::value, "недопустимый тип");
...
public:
	...
	matrix<cdim, T> operator* (int) const;
	matrix<cdim, T> operator= (int);
	matrix<cdim, T>& operator+= (const matrix<cdim, T>& );
	matrix<cdim, T> operator/ (const matrix<cdim, T>&) const;
	...
	friend ostream& operator<< (ostream& out, matrix<cdim, U> mtr);
	template<int cdim, class U>
	friend ostream& operator<< (ostream& out, matrix<cdim, fraction<U>> mtr);
	template<int cdim, class U>
...
ostream& operator<< (ostream& out, matrix<cdim, T> mtr)
{
	for (int i = 0; i < cdim; i++)
	{
		for (int j = 0; j < cdim; j++)
			out << (int)mtr.m_mtr[i][j] << ' ';
...
//для вывода дроби
template <int cdim, class T>
ostream& operator<< (ostream& out, matrix<cdim, fraction<T>> mtr)
{
	for (int i = 0; i < cdim; i++)
	{
		for (int j = 0; j < cdim; j++)
...
template<int cdim, class T>
inline matrix<cdim, T> matrix<cdim, T>::operator* (int num) const
{
...
template<int cdim, class T>
inline matrix<cdim, T> matrix<cdim, T>::operator= (int num)
{
	for (int i = 0; i < cdim; i++)
		for (int j = 0; j < cdim; j++)
			this->m_mtr[i][j] = num;
