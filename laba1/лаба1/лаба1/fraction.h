...
class fraction
{
	...
	static_assert(std::is_same<T, int>::value || std::is_same<T, char>::value || std::is_same<T, unsigned char>::value, "недопустимый тип");
...
template<class T>
inline fraction<T> fraction<T>::operator* (int num) const
{
	fraction<T> newnum;
...
template<class T>
inline fraction<T> fraction<T>::operator/ (float num) const
{
	fraction newfr;
	newfr.m_nmr = this->m_nmr;
...
template<class T>
inline fraction<T>& fraction<T>::operator=(int num)
{
	this->m_nmr = num;
	this->m_dnm = 1;
...
{
	if ( ((float)m_nmr / (float)m_dnm) == (float)num )
		return true;
	else
		return false;
}
